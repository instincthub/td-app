					...
					<div className="m-b-50 mb-7 pt-4">
						<div className="mt-10">
							...
							<div className="signup__list">
								<p>
									Scored: {data.score} | Try count: {data.try_count}
								</p>
								<h2 className="sub_header">Got {data.points} points</h2>

								<p className="mt-3">
									{data.met_expectation
										? "Congratulation, you met expectation! Keep Learning"
										: "You didn't have enough point. Relearn and come try again"}
									{data.tdn === false ? (
										<strong>
											{" "}
											<Link to={`/quiz/quiztest/?slug=${data.slug}`}>
												{data.step ? "Retake Quiz" : ""}
											</Link>
										</strong>
									) : (
										""
								...
								<div className="mt-4">
									<a
										href={
											data.met_expectation === true && data.tdn === true
												? data.whatsapp_group
												: "https://chat.whatsapp.com/IvuvWu6DtwR3PWp0mUWJRn"
										}
										target="_blank"
										rel="noreferrer"
									>
										<button className="primary-btn d-inline-block">
