...
const Register = () => {
	...
	useState(() => {
		...
		fetchAPI(
			setCohort,
			HOST_URL() + "/api/v1/assessments/skills/cohort/?data=NOW",
			requestOptions,
			true
		);
		...
		if (getCookie("u_id")) {
			...
			fetchAPI(
				setStudent,
				HOST_URL() +
					"/api/v1/auth/tdn_register/details/get/" +
					getCookie("u_id"),
				requestOptions,
				true
			);
	...
	useEffect(() => {
		...
		if (document.getElementById("id_school")) {
			...
			autocomplete(
				document.getElementById("id_faculty"),
				customData["faculties"]
			);
		}
	...
	if (items && items.message === "updated")
		window.location.href = "/quiz/?slug=" + items.assessment;
	else if (!access && items && status === "success")
	...
	const removeErrMsg = () => {
		...
		if (ERR) {
			ERR.forEach((option) => {
				...
				if (option.querySelector(".field .err_msg"))
					option.querySelector(".field .err_msg").remove();
	...
	const hideFields = (e, lists) => {
		...
		if (e.getAttribute("type") && e.value.toLowerCase() === "true")
			fields("block");
		else if (e.getAttribute("type") && e.value.toLowerCase() === "false")
			fields("none");
	};
	...
	const postData = (form) => {
		...
		formData.set(
			"invalid_token_url",
			window.location.origin + "/register/invalid-token"
		);
		...
		let access_url = access
			? `${HOST_URL()}/api/v1/auth/tdn_register/details/put/${getCookie(
					"u_id"
			  )}/`
			: HOST_URL() + "/api/v1/auth/tdn_register/";
	...
	if (cohort && cohort.count !== 0) {
			...
			<div>
				...
				>
					...
					<div className="container">
						<div className="diversity_assessment">
							...
							<p>
								With your dedication, we believe this programme will transform
								your life in 12 weeks!{" "}
								<strong>
									{" "}
									We are giving you a value worth over N250k ($500) for free.{" "}
								</strong>
								The only way you can show appreciation is to become successful
								with it.{" "}
							</p>

							<p>
								{" "}
								This application will take roughly 30 minutes (plus the
								assessment). Immediately after the test, you will know whether
								you meet expectations.
							</p>

							<p>
								No need to panic, the assessment for beginner cohorts doesn't
								have any technical questions; we only want to know whether you
								can read and write.{" "}
							</p>

							<p>
								Please try and be as descriptive as possible and ensure all
								links can be accessed by a representative from InstinctHub using
								an <strong>@instincthub.com</strong> domain.
							</p>

							<p>
								Kindly follow all the social media handles below. While we
								encourage you to follow these pages for continuous updates, it
								is not part of the prerequisite:
							</p>
					...
					<section className="container">
						<div className="diversity_data register">
							{access === null ? (
								<div className="mb-5">
									<Link className="inline-link" to="/login/?next=/register">
							...
							<RadioField
								options={courseChoice ? courseChoice.results : []}
								names="course_choice"
							...
							<div className="diversity_data educational_bg">
								...
								<TextField
									...
									label="Income Expectation (eg. N10,000)"
									maxLengths="250"
								/>
              ...
              * *"
								...
								<h5 className="top_form">
									On a scale of 1 - 10 (one being being not really important and
									ten being the most important - how important is this skill for
									you)?
								</h5>
								<TextField
									types="number"
								...
								<div className="upload_img">
									<FileInputField
										...
										requireds={true}
										titles={
											"Upload valid ID (NIN, Drivers License, Passport etc. *"
										}
										accepts={"image/*"}
									/>
								</div>
		...
		<div className="container">
			<div className="reg_closed">
				...
				<h3>Registration has been closed.</h3>
				<p>
					Registration for this programme has been closed as we are currently in
					session.
				</p>
				<p>We appreciate your interest!</p>
				<br />
