...
function QuizTest() {
    ...
    useEffect(() => {
        ...
        fetAPI(setData, HOST_URL()+"/assessment/quiztest/1/", reqOptions("GET"), true)

    }, [])

    if (data['a_assessment'] && currentPage > 0) { // Check if data
        ...
        const submitAnswerToDB = (e) =>{
            ...
            if (e.target.getAttribute('type') === "button" || e.target.id === "btn_label") {
                ...
                fetAPI(setData, HOST_URL()+"/assessment/answer/", reqOptions("POST", JSON.stringify(answerData)), true)
