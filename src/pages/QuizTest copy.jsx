...
function QuizTest() {
	...
	useEffect(() => {
		...
		if (typeof assessment === "object") {
			...
			setCurrent_records(
				assessment.slice(indexOfFirstRecord, indexOfLastRecord)
			); // single object
			...
			if (
				localStorage.getItem("quiz_duration_min") === null ||
				localStorage.getItem("quiz_duration_sec") === null ||
				localStorage.getItem("quiz_duration_min") === "undefined"
			) {
	...
	return (
		<div>
			...
			<div
				...
				onClick={(e) => {
					...
					if (
						...
						e.target.id === "btn_label"
					) {
						const optionInputs =
							document.querySelectorAll('input[name="quiz"]');
						optionInputs.forEach((e) => {
							// Check for user selected option
							if (e.checked === true) {
			...
			>
				<h3>
					Question {currentPage} of {nPages}:
				</h3>
