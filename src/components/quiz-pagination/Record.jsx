...
import { getCookie, fetchAPI, reqOptions, HOST_URL } from '../static/assets/js/help_func';
...
const Records = ({data, dataArray, ...props}) => {
    ...
    const nextPage = () => {
        if(props.currentPage < Math.ceil(dataArray.length / props.itemsPerPage) - 1) {
        ...
        if(props.currentPage === props.nPages-1){
            ...
            navigate(`/quiz/quiztest/results/?user_id=${getCookie('u_id')}&assessment_id=${data.assessment}&counts=${props.try_count}`)
        }
    }
    ...
    const submitAnswerToDB = (e) =>{
        ...
        fetchAPI(props.setSubmitAnswer, HOST_URL()+"/api/v1/assessment/answer/", requestOptions, true, props.setStatus)

        // Update the setShowResults state in PaginationQuiz
        if (showResults) { // redirect to results
  ...
  return (  
        <div>
            ...
            <form role="form" id="quizform" name="quizform" 
                // data-end={props.currentPage === props.nPages ? true : false}
                onSubmit={(e)=>{
                    e.preventDefault();
                    submitAnswerToDB(e.target)
                }}>
                <input type="number" hidden name="assessment" defaultValue={data.assessment}/>
                <input type="number" hidden name="user" defaultValue={getCookie('u_id')}/>
                ...
                <div>
                    ...
                    {data.q_option.map((item, index)=> (
                        <div key={index}>
                        {index === 1 ?  <input type="number" hidden name="question" defaultValue={item.question}/> : ''}
                        <label 
